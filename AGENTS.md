...
- `vp run dev` starts server and web. In a worktree, state defaults to that worktree's gitignored `.t3`, which deliberately outranks an ambient `T3CODE_HOME` so you cannot land on shared state by accident. An explicit `--home-dir` still wins. **That default lives in the dev runner, not the server.** Booting `node apps/server/src/bin.ts serve` by hand bypasses it entirely and follows the ambient `T3CODE_HOME` — which in a cockpit worktree is the developer's live install — so always pass `--base-dir "$PWD/.t3"` when smoke-testing a server directly (rule 2 below).
