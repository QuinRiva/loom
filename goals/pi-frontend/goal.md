> **NON-AUTHORITATIVE (2026-06-23):** This Markdown file is no longer a source
> of truth. Goals and tasks are now DB-authoritative and event-sourced
> (see `plans/db-goals-and-tasks-migration.md`). The agent updates tasks via
> the `t3 goal task ...` CLI, not by editing this file. Kept as a planning/
...
- [ ] Migrate goals and task lists from file-backed `goal.md` into DB-authoritative orchestration state
...
- [ ] Future: allow safe reuse/recovery of deleted goal slugs (MVP reserves deleted slugs to keep DB constraints simple)
