...
const files = fs
  .readFileSync("/tmp/conf.txt", "utf8")
  .split("\n")
  .map((l) => l.trim())
  .filter((f) => f && !PROTECT.test(f));
const ledger = [];
...
for (const f of files) {
  ...
  for (const mode of ["union", "ours", "theirs"]) {
    ...
    if (await parses(out, f)) {
      chosen = mode;
      NodeFS.writeFileSync(f, out);
      break;
    }
  }
  if (!chosen) {
    counts.failed++;
    ledger.push({ file: f, mode: "FAILED", hunks: hunks.length });
    continue;
  }
  counts[chosen]++;
  ledger.push({
    ...
    dropped:
      chosen === "union"
        ? []
        : hunks.map((h) => (chosen === "ours" ? h.theirs : h.ours)).filter((t) => t.trim()),
  });
}
NodeFS.writeFileSync(".artifacts/pull7-autoresolve-ledger.json", JSON.stringify(ledger, null, 1));
