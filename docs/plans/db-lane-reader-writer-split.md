...
- **No transactions on the reader.** The reader client's `transactionAcquirer` dies with a defect. Each read statement gets a fresh WAL read snapshot — statement-level consistency, which is what every routed consumer needs (multi-statement snapshot assembly tolerates commits landing between statements today already, because those reads run interleaved with writes on the shared connection; the reader does not weaken any existing guarantee).
