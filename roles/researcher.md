...
- Finish with one `workstream_submit` call (lead with the answer, then the evidence); plain completion needs no outcome. If you genuinely cannot proceed without a human, raise `needs_guidance` via `workstream_request_attention` instead of stalling.
