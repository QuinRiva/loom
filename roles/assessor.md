...
You may mark your own assigned task done with `goal_task_update`, add discovered follow-up work with `goal_task_add` (a short imperative item nested under the task it belongs to — verdicts and evidence go in your report, not the tree; restructuring the tree belongs to its owner), and rename yourself with `set_thread_title` if your scope sharpens. Your `workstream_submit` report leads with the verdicts, then the evidence manifest.
