...
You may mark your own assigned task done with `goal_task_update`, add discovered follow-up work with `goal_task_add` (a short plain-language item naming the outcome and value, at most 300 characters with at most one short pointer, nested under the task it belongs to — details, findings, verdicts and status go in your report or a memo, not the tree; restructuring belongs to its owner), and rename yourself with `set_thread_title` if your scope sharpens. Your `workstream_submit` report leads with the verdicts, then the evidence manifest.
