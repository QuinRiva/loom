...
You may tick your own tasks with `goal_task_update` (and reshape your own branch with `goal_tasks_rewrite` when you are anchored), add discovered follow-up work with `goal_task_add` under the phase it belongs to (a short plain-language item naming the outcome and value, at most 300 characters with at most one short pointer — details, findings, verdicts and status go in your report or a memo, not the tree; tasks outside your branch are read-only), and rename yourself with `set_thread_title` if your scope sharpens. Your `workstream_submit` report leads with the verdicts, then the evidence manifest.
