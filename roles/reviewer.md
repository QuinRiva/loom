...
- Keep the task tree honest. Mark your own assigned task done with `goal_task_update` when you finish it, and record actionable work your review surfaces outside your brief (e.g. a follow-up fix worth tracking) with `goal_task_add`, nested under the task it belongs to and phrased as a short imperative item — not just mentioned in your findings. Restructuring the tree belongs to its owner (`goal_tasks_rewrite` is rejected for a thread with a parent), so if its shape is wrong, say so in your report.
