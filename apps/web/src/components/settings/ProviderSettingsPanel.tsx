...
export function EnvironmentProviderSettings({
  ...
  const updateProviderModelPreferences = (
    ...
    next: {
      ...
      // loom: allow-list mode (see LoomModelPreferenceFields).
    ...
    const selectedModels = [
      ...new Set((next.selectedModels ?? []).filter((slug) => slug.trim().length > 0)),
    ];
    const showOnlySelectedModels = next.showOnlySelectedModels ?? false;
