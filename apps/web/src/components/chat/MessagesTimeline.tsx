...
const EMPTY_HANDOFF_RECEIPTS: ReadonlyArray<HandoffReceiptView> = []; // loom:
...
// loom: exported for the handoff-receipt row, which lives under `~/loom/`.
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  handoffReceipts = EMPTY_HANDOFF_RECEIPTS, // loom:
  ...
  const rawRows = useMemo(() => {
    ...
    // loom: splice receipts in at their submission time, after derivation so the
    // projection input upstream compares for row reuse stays upstream's.
    return insertHandoffReceiptRows(projection.rows, handoffReceipts);
  }, [
...
>({
  ...
  handlers: [
        ...
        return (
          ...
          >
            <UserMessageReviewCommentCard
              comment={{
                kind: "line", // loom: review-comment contexts are a line | mdx-anchor union here
                id: record.contextId,
    ...
    // loom: a `#`-mentioned thread, rendered by the same chip the composer uses.
    {
      kind: "thread",
      canRender: (record) => record.kind === "thread",
      render: (record, context) =>
        record.kind === "thread" ? (
          <ThreadContextChip reference={threadReferenceFromRecord(record)} />
        ) : (
          <UnavailableUserMessageContextChip {...context} />
        ),
    },
...
// loom: the card renders line comments; mdx-anchor ones have no diff to show.
function UserMessageReviewCommentCard({ comment }: { comment: LineReviewCommentContext }) {
  const ctx = use(TimelineRowCtx);
