...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  // recedes (the helper's default branch): it is the one thing in the inbox
  ...
  const handleThreadContextMenu = useCallback(
    (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      void (async () => {
        ...
        // loom: goal entries (create goal from thread / assign to goal) sit above
        // the copy/archive/delete tail. Spliced rather than hand-rolled so
        // upstream keeps sole ownership of the base list and its ordering.
        ...
        const goalInsertAt = menuItems.findIndex((item) => item.id === "copy-path");
        menuItems.splice(
          goalInsertAt < 0 ? menuItems.length : goalInsertAt,
          0,
          ...buildGoalMenuItems(
                      ...
                      const renderThreadRowInner = (
                        ...
                        return (
                          <SidebarThreadRow
                            ...
                            onFileDropThreads={handleThreadFileDrop}
                            // loom: this root's hidden sub-thread graph, rolled up.
                            graphRollup={graphRollupByThreadKey.get(threadKey) ?? null}
                          />
                        );
                      };
