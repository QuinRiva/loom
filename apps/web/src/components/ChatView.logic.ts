...
export function buildLocalDraftThread(
  ...
  return {
    ...
    // loom: the fork's required thread fields. `goalId` and `forkFromThreadId`
    // carry the draft's own values so a "New session" started from the Goal
    // panel keeps its goal and a fork stays a fork; a local draft is never a
    // `goal_continue` successor, so `continuesThreadId` stays null.
    ...loomThreadDefaults,
