...
import { ProjectionGoal, ProjectionGoalTask } from "../../persistence/Services/ProjectionGoals.ts";
...
function assembleGoals(
  goalRows: ReadonlyArray<ProjectionGoal>,
  taskRows: ReadonlyArray<ProjectionGoalTask>,
): OrchestrationGoal[] {
  const activeTasksByGoal = new Map<string, FlatGoalTask[]>();
  for (const task of taskRows) {
    ...
    const bucket = activeTasksByGoal.get(task.goalId);
    if (bucket) bucket.push(flat);
    else activeTasksByGoal.set(task.goalId, [flat]);
  }
...
function toGoalShells(goals: ReadonlyArray<OrchestrationGoal>): OrchestrationGoalShell[] {
  return goals
...
const ProjectionProjectDbRowSchema = ProjectionProject.mapFields(
  Struct.assign({
    ...
    defaultStartFromOrigin: Schema.NullOr(Schema.fromJsonString(Schema.Boolean)),
 ...
 * Bounds peak memory + outbound payload when opening/re-subscribing a long-lived
 * thread; older activities are fetched on demand (lazy-load) and live ones stream
 * in via events.
 */
...
// is only equivalent to the old unbounded read when `beforeSequence` is
// non-negative — a negative cursor would silently match no sequenced rows and
// return only unsequenced ones. Validating here (not just at the RPC boundary)
// keeps any future non-RPC caller honest.
...
const ACTIVITY_PREVIEW_MAX_LENGTH = 200;

/** Collapse a multi-line assistant narration into a single bounded preview line. */
function normalizeActivityPreview(text: string): string | null {
...
function mapProjectShellRow(
  ...
  return {
    ...
    defaultStartFromOrigin: row.defaultStartFromOrigin,
...
function mapLeanThreadShellRow(
  ...
  return {
    ...
    ...mapThreadPullRequests(
      ...
      row.projectId,
      derived.repositoryIdentity ?? undefined,
    ),
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listGoalTaskRows = SqlSchema.findAll({
    Request: Schema.Void,
  ...
  // Latest assistant-narration text per active thread, for the Workstream card's
  // one-line activity preview. Window-function pick of the most recent
  // non-empty assistant message keeps this to one row per thread.
  const listActiveLatestAssistantMessageRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionLatestAssistantMessageRowSchema,
    execute: () =>
      sql`
        SELECT "threadId", text
        FROM (
          SELECT
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          -- it fails the decode at runtime, where typecheck cannot see it, and
          -- every ingested provider event for a thread with a session is then
          -- dropped.
          sessions.last_error_class AS "lastErrorClass",
          sessions.updated_at AS "updatedAt"
        FROM projection_threads AS threads
  ...
  // can't blow the server heap or the outbound subscribe payload. New activities
  // still stream in live via the event subscription; older history is fetched on
  // demand via getThreadActivitiesPage. Select the most recent N (sequence DESC,
  // NULLs last = oldest legacy rows last) then re-sort ascending for display.
  ...
  // `(sequence < beforeSequence OR sequence IS NULL)` lets the
  ...
  const getLatestAssistantMessageRowByThread = SqlSchema.findOneOption({
    Request: ThreadIdLookupInput,
...
pending_approval_requests AS (
      ...
      .pipe(
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              const goalRows = yield* listGoalRows(undefined).pipe(
                Effect.mapError(
                  toPersistenceSqlOrDecodeError(
              ...
              const goalTaskRows = yield* listGoalTaskRows(undefined).pipe(
                Effect.mapError(
                  toPersistenceSqlOrDecodeError(
                    "ProjectionSnapshotQuery.getSnapshot:listGoalTasks:query",
                    "ProjectionSnapshotQuery.getSnapshot:listGoalTasks:decodeRows",
                  ),
                ),
              );
              ...
              const snapshot = {
                ...
                projects,
                goals: assembleGoals(goalRows, goalTaskRows),
                threads,
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listGoalTaskRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listGoalTasks:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listGoalTasks:decodeRows",
              ),
            ),
          ),
          Effect.suspend(() =>
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listActiveLatestAssistantMessageRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listLatestAssistantMessages:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listLatestAssistantMessages:decodeRows",
              ),
            ),
          ),
        ]),
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const goalTaskRows = yield* listGoalTaskRows(undefined).pipe(
                Effect.mapError(
                  toPersistenceSqlOrDecodeError(
                    "ProjectionSnapshotQuery.getShellSnapshot:listGoalTasks:query",
                    "ProjectionSnapshotQuery.getShellSnapshot:listGoalTasks:decodeRows",
                  ),
                ),
              );
              ...
              const pullRequestsByThread = groupPullRequestRowsByThread(pullRequestRows);
              const lastActivityPreviewByThread = new Map(
                Arr.filterMap(latestAssistantMessageRows, (row) => {
                  const preview = normalizeActivityPreview(row.text);
                  return preview === null
                    ? Result.failVoid
                  ...
                  row.deletedAt === null
                    ? Result.succeed({
                        ...mapLeanThreadShellRow(row, sessionByThread, latestTurnByThread, {
                          ...
                          repositoryIdentity: repositoryIdentities.get(row.projectId),
                        }),
                        brief: row.brief,
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    defaultStartFromOrigin: option.value.defaultStartFromOrigin,
  ...
  const getGoalShellById: ProjectionSnapshotQueryShape["getGoalShellById"] = (goalId) =>
    Effect.gen(function* () {
      const goalRows = yield* listGoalRows(undefined).pipe(
        Effect.mapError(
          toPersistenceSqlOrDecodeError(
            "ProjectionSnapshotQuery.getGoalShellById:listGoals:query",
            "ProjectionSnapshotQuery.getGoalShellById:listGoals:decodeRows",
          ),
        ),
      );
      const goalTaskRows = yield* listGoalTaskRows(undefined).pipe(
        Effect.mapError(
          toPersistenceSqlOrDecodeError(
            "ProjectionSnapshotQuery.getGoalShellById:listGoalTasks:query",
            "ProjectionSnapshotQuery.getGoalShellById:listGoalTasks:decodeRows",
          ),
        ),
      );
      const shell = toGoalShells(assembleGoals(goalRows, goalTaskRows)).find(
        (goal) => goal.id === goalId,
      );
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        gateRounds: threadRow.value.gateRounds,
        pendingRework: threadRow.value.pendingRework > 0,
        lastOutcome: threadRow.value.lastOutcome,
        ...
        planLaneSince: threadRow.value.planLaneSince,
        dependenciesSince: threadRow.value.dependenciesSince,
        faninSince: threadRow.value.faninSince,
