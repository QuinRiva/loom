...
const EXTENSION_SOURCE = String.raw`export default function(pi) {
  ...
  pi.registerTool({
    ...
    promptSnippet: "submit your report + outcome in one terminal call: plain completion → done; 'needs_human' → human flag; any other outcome → routed, with unmatched non-rework outcomes yielding to your orchestrator.",
    promptGuidelines: [
    ...
    parameters: {
      ...
      properties: {
        ...
        outcome: { type: "string", description: "Structured outcome token. Omitted ⇒ 'done' (plain completion). Reserved: 'done', 'needs_human'. Review-gate verdicts (reviewers): 'clean', 'fixed_inline', 'needs_rework'. During an active rework round, any non-'needs_human' token routes back to the reviewer; otherwise any unmatched token yields you to the orchestrator — use a short snake_case token like 'rework_approach'." },
