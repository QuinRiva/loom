  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        // loom: prepend the session worktree's node_modules/.bin so the Claude
        // query child resolves that worktree's workspace binaries before
        // anything inherited from the server's PATH. claudeEnvironment is the
        // full env; upstream's agent-device overlay wraps the result.
