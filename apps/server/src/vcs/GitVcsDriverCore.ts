...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const invalidateStatusStaticCaches = (cwd: string) =>
    Effect.gen(function* () {
      ...
      // loom keys the default-branch cache by (gitCommonDir, remoteName).
      yield* Cache.invalidate(
        defaultBranchCache,
        new StatusDefaultBranchCacheKey({ gitCommonDir: cacheKey, remoteName: "origin" }),
      );
  ...
  const readStatusDetailsRemote = Effect.fn("readStatusDetailsRemote")(function* (
    ...
    // loom: the batch reader resolves upstream once per repository and hands it
    // in; only the single-cwd path pays for its own lookup.
    const upstream =
